...
from graphviz import Digraph  # noqa: E402 — must import after PATH setup
...
with dot.subgraph(name="cluster_auth") as sub:
    ...
    sub.node(
        "School",
        "School|{code|subdomain|name|address|is_active|subscription_tier|l<logo>}",
        fillcolor="#0f3460",
        fontcolor="white",
    )
    sub.node(
        "User",
        "{User|{role|email|first_name|last_name|is_active<is_active>|email_verified|two_factor_enabled|l<school>}}",
        fillcolor="#16213e",
        fontcolor="white",
    )
    sub.node(
        "AuditLog",
        "{AuditLog|{action|resource_type|resource_id|ip_address|l<school> l<user>}}",
        fillcolor="#1a1a2e",
        fontcolor="white",
    )
    sub.node(
        "UserSession",
        "{UserSession|{refresh_token_jti|l<user>|device_info|is_active}}",
        fillcolor="#1a1a2e",
        fontcolor="white",
    )
    sub.node(
        "PasswordResetToken",
        "{PasswordResetToken|{token|l<user>|expires_at|used}}",
        fillcolor="#1a1a2e",
        fontcolor="white",
    )
    sub.node(
        "EmailVerificationToken",
        "{EmailVerificationToken|{token|l<user>|email|expires_at|used}}",
        fillcolor="#1a1a2e",
        fontcolor="white",
    )
    sub.node(
        "TwoFactorBackupCode",
        "{TwoFactorBackupCode|{hashed_code|l<user>|used}}",
        fillcolor="#1a1a2e",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_students") as sub:
    ...
    sub.node(
        "AcademicYear",
        "{AcademicYear|{name|start_date|end_date|is_current|l<school>}}",
        fillcolor="#0d1b2a",
        fontcolor="white",
    )
    ...
    sub.node(
        "Classroom",
        "{Classroom|{name|capacity|room_number|l<school> l<grade> l<academic_year> l<class_teacher>}}",
        fillcolor="#1b263b",
        fontcolor="white",
    )
    sub.node(
        "Student",
        "{Student|{admission_number|roll_number|date_of_birth|gender|l<school> l<user>|is_active}}",
        fillcolor="#0d1b2a",
        fontcolor="white",
    )
    sub.node(
        "Guardian",
        "{Guardian|{first_name|last_name|email|phone|l<user>|is_primary}}",
        fillcolor="#1b263b",
        fontcolor="white",
    )
    sub.node(
        "StudentGuardian",
        "{StudentGuardian|{l<student> l<guardian>|relationship|is_primary_contact|portal_access}}",
        fillcolor="#0d1b2a",
        fontcolor="white",
    )
    sub.node(
        "Enrollment",
        "{Enrollment|{l<student> l<classroom> l<academic_year>|status|is_active}}",
        fillcolor="#1b263b",
        fontcolor="white",
    )
    sub.node(
        "ParentProfile",
        "{ParentProfile|{l<user> l<school>|relationship|occupation|l<guardian>}}",
        fillcolor="#1b263b",
        fontcolor="white",
    )
    sub.node(
        "Document", "{Document|{l<student>|document_type|title|l<uploaded_by>}}", fillcolor="#0d1b2a", fontcolor="white"
    )
    ...
    sub.edge("User", "ParentProfile", arrowhead="odiamond", label="1:1")
    sub.edge("Guardian", "ParentProfile", arrowhead="odiamond", label="1:1")
    sub.edge("School", "ParentProfile", arrowhead="crow", label="1:N")
...
with dot.subgraph(name="cluster_academics") as sub:
    ...
    sub.node(
        "Subject",
        "{Subject|{name|code|is_core|is_elective|max_marks|l<school> l<grade>}}",
        fillcolor="#16213e",
        fontcolor="white",
    )
    sub.node(
        "TeacherAssignment",
        "{TeacherAssignment|{l<teacher> l<subject> l<classroom> l<academic_year>|is_primary}}",
        fillcolor="#1a1a2e",
        fontcolor="white",
    )
    sub.node(
        "TeacherProfile",
        "{TeacherProfile|{employee_id|qualification|specialization|department|l<user> l<school>}}",
        fillcolor="#16213e",
        fontcolor="white",
    )
    sub.node(
        "LessonPlan",
        "{LessonPlan|{title|topic|date|duration|status|l<assignment>}}",
        fillcolor="#1a1a2e",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_attendance") as sub:
    ...
    sub.node(
        "AttendanceRecord",
        "{AttendanceRecord|{l<student> l<classroom> l<academic_year>|date|status|l<recorded_by>}}",
        fillcolor="#3c096c",
        fontcolor="white",
    )
    sub.node(
        "PeriodAttendance",
        "{PeriodAttendance|{l<student> l<assignment>|date|period_number|status|l<recorded_by>}}",
        fillcolor="#5a189a",
        fontcolor="white",
    )
    sub.node(
        "AttendanceLeave",
        "{AttendanceLeave|{l<student>|leave_type|from_date|to_date|reason|status|l<reviewed_by>}}",
        fillcolor="#7b2cbf",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_gradebook") as sub:
    ...
    sub.node(
        "GradingScaleEntry",
        "{GradingScaleEntry|{grade_letter|min_percentage|max_percentage|grade_point|l<scale>}}",
        fillcolor="#14919b",
        fontcolor="white",
    )
    ...
    sub.node(
        "Exam",
        "{Exam|{name|start_date|end_date|status|l<school> l<academic_year> l<exam_type> l<created_by>}}",
        fillcolor="#14919b",
        fontcolor="white",
    )
    sub.node(
        "ExamSchedule",
        "{ExamSchedule|{date|start_time|end_time|venue|max_marks|l<exam> l<subject> l<classroom> l<invigilator>}}",
        fillcolor="#0d7377",
        fontcolor="white",
    )
    sub.node(
        "Grade",
        "{Grade|{l<student> l<exam_schedule>|marks_obtained|is_absent|is_pass|l<graded_by>}}",
        fillcolor="#14919b",
        fontcolor="white",
    )
    sub.node(
        "Assessment",
        "{Assessment|{title|assessment_type|due_date|max_marks|l<assignment>}}",
        fillcolor="#0d7377",
        fontcolor="white",
    )
    sub.node(
        "AssessmentSubmission",
        "{AssessmentSubmission|{l<assessment> l<student>|marks_obtained|submitted_at|file|is_late}}",
        fillcolor="#14919b",
        fontcolor="white",
    )
    sub.node(
        "ReportCard",
        "{ReportCard|{l<student> l<exam> l<academic_year>|percentage|gpa|rank_in_class|status}}",
        fillcolor="#0d7377",
        fontcolor="white",
    )
    sub.node(
        "GradeChangeLog",
        "{GradeChangeLog|{l<grade>|old_marks|new_marks|reason|changed_at|l<changed_by>}}",
        fillcolor="#14919b",
        fontcolor="white",
    )
    sub.node(
        "GradeChangeProposal",
        "{GradeChangeProposal|{l<grade>|proposed_marks|status|reason|l<proposed_by>|reviewed_by}}",
        fillcolor="#0d7377",
        fontcolor="white",
    )
    ...
    sub.edge("Grade", "GradeChangeLog", arrowhead="crow", label="1:N")
    sub.edge("Grade", "GradeChangeProposal", arrowhead="crow", label="1:N")
    sub.edge("User", "GradeChangeLog", arrowhead="crow", label="1:N", headlabel="(changed_by)")
    sub.edge("User", "GradeChangeProposal", arrowhead="crow", label="1:N", headlabel="(proposed_by)")
...
with dot.subgraph(name="cluster_fees") as sub:
    ...
    sub.node(
        "FeeCategory", "{FeeCategory|{name|is_mandatory|recurrence|l<school>}}", fillcolor="#1b4332", fontcolor="white"
    )
    sub.node(
        "FeeStructure",
        "{FeeStructure|{amount|due_day|late_fee_per_day|l<school> l<academic_year> l<grade> l<fee_category>}}",
        fillcolor="#2d6a4f",
        fontcolor="white",
    )
    sub.node(
        "FeeInvoice",
        "{FeeInvoice|{invoice_number|l<student> l<academic_year>"
        " l<fee_structure>|due_date|base_amount|total_amount|"
        "paid_amount|status|l<created_by>}}",
        fillcolor="#40916c",
        fontcolor="white",
    )
    sub.node(
        "Payment",
        "{Payment|{amount|payment_method|status|transaction_id|receipt_number|l<invoice> l<collected_by>}}",
        fillcolor="#52b788",
        fontcolor="white",
    )
    sub.node(
        "Scholarship",
        "{Scholarship|{name|discount_type|discount_value|l<school> l<student> l<academic_year>|reason|l<approved_by>}}",
        fillcolor="#2d6a4f",
        fontcolor="white",
    )
    sub.node(
        "PaymentGatewayConfig",
        "{PaymentGatewayConfig|{l<school>|stripe_enabled|khalti_enabled|esewa_enabled}}",
        fillcolor="#40916c",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_timetable") as sub:
    ...
    sub.node(
        "Period",
        "{Period|{name|period_number|start_time|end_time|is_break|l<school>}}",
        fillcolor="#e07a5f",
        fontcolor="white",
    )
    sub.node(
        "TimetableSlot",
        "{TimetableSlot|{day_of_week|l<classroom> l<period> l<subject> l<teacher> l<academic_year>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
    sub.node(
        "SchoolEvent",
        "{SchoolEvent|{title|description|start_datetime|end_datetime|event_type|l<school>}}",
        fillcolor="#e07a5f",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_communication") as sub:
    ...
    sub.node(
        "Announcement",
        "{Announcement|{title|content|priority|audience|is_draft|l<school> l<created_by>}}",
        fillcolor="#6a994e",
        fontcolor="white",
    )
    sub.node(
        "AnnouncementRead",
        "{AnnouncementRead|{l<announcement> l<user>|read_at}}",
        fillcolor="#81b29a",
        fontcolor="white",
    )
    sub.node(
        "DirectMessage",
        "{DirectMessage|{subject|body|l<sender> l<recipient>|read_at}}",
        fillcolor="#a7c957",
        fontcolor="white",
    )
    sub.node(
        "NotificationTemplate",
        "{NotificationTemplate|{event_type|email_subject|email_body|l<school>|is_active}}",
        fillcolor="#6a994e",
        fontcolor="white",
    )
    sub.node(
        "Notification",
        "{Notification|{title|body|notification_type|l<user>|is_read}}",
        fillcolor="#81b29a",
        fontcolor="white",
    )
    sub.node(
        "DeviceToken", "{DeviceToken|{token|device_type|l<user>|is_active}}", fillcolor="#a7c957", fontcolor="white"
    )
...
with dot.subgraph(name="cluster_hr") as sub:
    ...
    sub.node(
        "Department",
        "{Department|{name|code|description|head_of_dept|l<school>}}",
        fillcolor="#3d405b",
        fontcolor="white",
    )
    sub.node(
        "Employee",
        "{Employee|{employee_id|designation|department|joining_date|l<user> l<school>}}",
        fillcolor="#5c5f7e",
        fontcolor="white",
    )
    sub.node(
        "SalaryStructure",
        "{SalaryStructure|{title|basic_pay|allowances|deductions|l<school> l<department>}}",
        fillcolor="#3d405b",
        fontcolor="white",
    )
    sub.node(
        "EmployeeSalary",
        "{EmployeeSalary|{l<employee> l<salary_structure>|effective_from|is_active}}",
        fillcolor="#5c5f7e",
        fontcolor="white",
    )
    sub.node(
        "Payslip",
        "{Payslip|{month|year|gross_pay|net_pay|deductions|l<employee_salary>}}",
        fillcolor="#3d405b",
        fontcolor="white",
    )
    sub.node(
        "LeaveRequest",
        "{LeaveRequest|{leave_type|from_date|to_date|reason|status|l<employee> l<approved_by>}}",
        fillcolor="#5c5f7e",
        fontcolor="white",
    )
    sub.node(
        "AccountantProfile",
        "{AccountantProfile|{l<user> l<school>|certification|specialization|l<department>}}",
        fillcolor="#3d405b",
        fontcolor="white",
    )
    ...
    sub.edge("User", "AccountantProfile", arrowhead="odiamond", label="1:1")
    sub.edge("School", "AccountantProfile", arrowhead="crow", label="1:N")
    sub.edge("Department", "AccountantProfile", arrowhead="crow", label="1:N")
...
with dot.subgraph(name="cluster_transport") as sub:
    ...
    sub.node(
        "Vehicle",
        "{Vehicle|{registration_number|type|capacity|make|model|year|l<school>}}",
        fillcolor="#e63946",
        fontcolor="white",
    )
    sub.node(
        "Driver",
        "{Driver|{license_number|phone|emergency_contact|l<user> l<school>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
    sub.node(
        "Route",
        "{Route|{name|start_point|end_point|distance|fare_amount|l<school>}}",
        fillcolor="#e63946",
        fontcolor="white",
    )
    sub.node(
        "RouteStop", "{RouteStop|{name|stop_order|latitude|longitude|l<route>}}", fillcolor="#f4a261", fontcolor="white"
    )
    sub.node(
        "StudentRoute",
        "{StudentRoute|{pickup_stop|dropoff_stop|fare|is_active|l<student> l<route> l<academic_year>}}",
        fillcolor="#e63946",
        fontcolor="white",
    )
    sub.node(
        "VehicleMaintenance",
        "{VehicleMaintenance|{maintenance_type|description|cost|service_date|next_service_date|l<vehicle>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_inventory") as sub:
    ...
    sub.node(
        "Supplier",
        "{Supplier|{name|contact_person|email|phone|address|l<school>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
    sub.node(
        "InventoryItem",
        "{InventoryItem|{name|sku|quantity|unit_price|reorder_level|l<school> l<category>}}",
        fillcolor="#e76f51",
        fontcolor="white",
    )
    sub.node(
        "StockMovement",
        "{StockMovement|{quantity|movement_type|reference|notes|l<item> l<created_by>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
    sub.node(
        "PurchaseOrder",
        "{PurchaseOrder|{order_number|status|total_amount|l<supplier> l<created_by>}}",
        fillcolor="#e76f51",
        fontcolor="white",
    )
    sub.node(
        "PurchaseOrderItem",
        "{PurchaseOrderItem|{quantity|unit_price|total_price|l<purchase_order> l<item>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_hostel") as sub:
    ...
    sub.node(
        "HostelRoom",
        "{HostelRoom|{room_number|capacity|occupied|fee_per_bed|l<hostel>}}",
        fillcolor="#2a9d8f",
        fontcolor="white",
    )
    sub.node(
        "HostelAllocation",
        "{HostelAllocation|{l<room> l<student> l<academic_year>|check_in|check_out|is_active}}",
        fillcolor="#264653",
        fontcolor="white",
    )
    sub.node(
        "HostelFee", "{HostelFee|{fee_type|amount|due_date|l<allocation>|paid}}", fillcolor="#2a9d8f", fontcolor="white"
    )
    sub.node(
        "HostelVisitor",
        "{HostelVisitor|{visitor_name|relationship|visit_date|in_time|out_time|l<allocation>}}",
        fillcolor="#264653",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_sports") as sub:
    ...
    sub.node(
        "Team", "{Team|{name|age_group|gender|l<sport> l<coach> l<school>}}", fillcolor="#3dbca8", fontcolor="white"
    )
    sub.node("TeamMember", "{TeamMember|{role|status|l<team> l<student>}}", fillcolor="#2a9d8f", fontcolor="white")
    sub.node(
        "SportEvent",
        "{SportEvent|{name|event_type|location|start_date|end_date|l<team> l<school>}}",
        fillcolor="#3dbca8",
        fontcolor="white",
    )
    sub.node(
        "SportAchievement",
        "{SportAchievement|{achievement_type|title|rank|date|description|l<team> l<student> l<school>}}",
        fillcolor="#2a9d8f",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_health") as sub:
    ...
    sub.node(
        "HealthRecord",
        "{HealthRecord|{blood_group|allergies|chronic_conditions|emergency_contact|l<student> l<school>}}",
        fillcolor="#e76f51",
        fontcolor="white",
    )
    sub.node(
        "NurseVisit",
        "{NurseVisit|{symptoms|diagnosis|treatment|medication|follow_up_date|l<student> l<record> l<attended_by>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
    sub.node(
        "Immunization",
        "{Immunization|{vaccine_name|date_given|dose_number|next_due_date|l<student> l<record>}}",
        fillcolor="#e76f51",
        fontcolor="white",
    )
    sub.node(
        "MedicationLog",
        "{MedicationLog|{medication_name|dosage|frequency|start_date|end_date|l<student> l<record>}}",
        fillcolor="#f4a261",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_library") as sub:
    ...
    sub.node(
        "Book",
        "{Book|{title|author|isbn|publisher|publish_year|total_copies|available_copies|l<school>}}",
        fillcolor="#9c89b8",
        fontcolor="white",
    )
    sub.node(
        "Checkout",
        "{Checkout|{l<book> l<student>|checked_out|due_date|returned_date|status|l<issued_by>}}",
        fillcolor="#b8a9c9",
        fontcolor="white",
    )
    sub.node(
        "LibrarianProfile",
        "{LibrarianProfile|{l<user> l<school>|certification|experience_years}}",
        fillcolor="#9c89b8",
        fontcolor="white",
    )
    ...
    sub.edge("User", "LibrarianProfile", arrowhead="odiamond", label="1:1")
    sub.edge("School", "LibrarianProfile", arrowhead="crow", label="1:N")
...
with dot.subgraph(name="cluster_conferences") as sub:
    ...
    sub.node(
        "ConferenceSlot",
        "{ConferenceSlot|{l<teacher> l<student>|date|start_time|end_time|status|mode|l<booked_by>}}",
        fillcolor="#b56576",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_behavior") as sub:
    ...
    sub.node(
        "Incident",
        "{Incident|{incident_type|description|date|location|severity|"
        "action_taken|l<student> l<reported_by> l<school>}}",
        fillcolor="#6c584c",
        fontcolor="white",
    )
    sub.node(
        "Referral",
        "{Referral|{reason|referral_type|notes|status|l<student> l<referred_by> l<school>}}",
        fillcolor="#8c7a6b",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_admissions") as sub:
    ...
    sub.node(
        "EnrollmentIntake",
        "{EnrollmentIntake|{name|start_date|end_date|capacity|is_open|l<school> l<grade> l<academic_year>}}",
        fillcolor="#1d3557",
        fontcolor="white",
    )
    sub.node(
        "Application",
        "{Application|{applicant_name|applicant_email|status|submitted_at|decision_date|l<intake> l<reviewed_by>}}",
        fillcolor="#457b9d",
        fontcolor="white",
    )
    sub.node(
        "ApplicationDocument",
        "{ApplicationDocument|{document_type|file|uploaded_at|l<application>}}",
        fillcolor="#1d3557",
        fontcolor="white",
    )
    sub.node(
        "ApplicationReview",
        "{ApplicationReview|{rating|comments|decision|l<application> l<reviewer>}}",
        fillcolor="#457b9d",
        fontcolor="white",
    )
    sub.node(
        "ApplicationTimelineEvent",
        "{ApplicationTimelineEvent|{l<application>|event_type|description|l<created_by>|created_at}}",
        fillcolor="#1d3557",
        fontcolor="white",
    )
    sub.node(
        "EntranceAssessment",
        "{EntranceAssessment|{l<application>|test_date|score|notes|l<assessed_by>}}",
        fillcolor="#457b9d",
        fontcolor="white",
    )
    ...
    sub.edge("Application", "ApplicationTimelineEvent", arrowhead="crow", label="1:N")
    sub.edge("Application", "EntranceAssessment", arrowhead="odiamond", label="1:1")
    sub.edge("User", "ApplicationTimelineEvent", arrowhead="crow", label="1:N", headlabel="(created_by)")
    sub.edge("User", "EntranceAssessment", arrowhead="crow", label="1:N", headlabel="(assessed_by)")
...
with dot.subgraph(name="cluster_alumni") as sub:
    ...
    sub.node(
        "AlumniProfile",
        "{AlumniProfile|{graduation_year|current_occupation|organization|l<user> l<school>}}",
        fillcolor="#457b9d",
        fontcolor="white",
    )
    sub.node(
        "AlumniEvent",
        "{AlumniEvent|{name|event_type|location|date|description|l<school>}}",
        fillcolor="#5c9cc9",
        fontcolor="white",
    )
    sub.node(
        "AlumniDonation",
        "{AlumniDonation|{amount|donation_type|date|payment_method|l<alumni> l<school>}}",
        fillcolor="#457b9d",
        fontcolor="white",
    )
    sub.node(
        "AlumniChapter",
        "{AlumniChapter|{name|city|country|description|l<school>}}",
        fillcolor="#5c9cc9",
        fontcolor="white",
    )
...
with dot.subgraph(name="cluster_cafeteria") as sub:
    ...
    sub.node(
        "MealMenu",
        "{MealMenu|{day_of_week|meal_type|main_item|side_item|price|l<school>}}",
        fillcolor="#d68c79",
        fontcolor="white",
    )
    sub.node(
        "MealPlan",
        "{MealPlan|{name|description|price_per_period|is_active|l<school>}}",
        fillcolor="#e0afa0",
        fontcolor="white",
    )
    sub.node(
        "MealBooking",
        "{MealBooking|{booking_date|meal_type|status|l<student> l<plan>}}",
        fillcolor="#d68c79",
        fontcolor="white",
    )
    sub.node(
        "DietaryRestriction",
        "{DietaryRestriction|{restriction_type|allergen|notes|l<student>}}",
        fillcolor="#e0afa0",
        fontcolor="white",
    )
