...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    # ``AbstractBaseUser`` does NOT provide these (only ``AbstractUser`` does),
    # but ~160 serializer fields and several views reference them — either as
    # ``source="user.get_full_name"`` or by calling it directly. Without them
    # DRF silently drops the field (read-only + missing attr → SkipField), so
    # every "…name" column rendered blank, and the direct calls raised
