...
class ReportingViewSet(viewsets.ViewSet):
    ...
    def at_risk_students(self, request):
        ...
        try:
            ...
            return Response({"error": "attendance_threshold must be a number"}, status=400)
        try:
        ...
        if academic_threshold:
            try:
                academic_threshold = float(academic_threshold)
            except (TypeError, ValueError):
                return Response({"error": "academic_threshold_pct must be a number"}, status=400)
    ...
    def export_attendance_pdf(self, request):
        ...
        from_date_str = request.query_params.get("from_date", str(timezone.localdate().replace(day=1)))
        to_date_str = request.query_params.get("to_date", str(timezone.localdate()))

        classroom = Classroom.objects.filter(id=classroom_id, school=school).first()
        if classroom is None:
            return Response({"error": "Classroom not found in your school."}, status=404)
        records = (
        ...
        elements.append(
            Paragraph(
                f"Generated by EduSphere SMS on {timezone.localdate().strftime('%B %d, %Y')}",
