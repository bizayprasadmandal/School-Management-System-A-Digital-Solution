...
class TestConferenceSlots:
    ...
    def test_book_slot(self, student_client, student_user, slot, school):
        from tests.factories import StudentFactory

        StudentFactory(user=student_user, school=school)
    ...
    def test_cancel_booking(self, slot, student_client):
        ...
        slot.student = pupil
        slot.save()
        r = student_client.post(f"{CONFERENCES_SLOTS}{slot.id}/cancel/")
        assert r.status_code == status.HTTP_200_OK
        slot.refresh_from_db()
    ...
    def test_complete_slot(self, admin_client, slot, teacher):
        ...
        assert r.status_code == status.HTTP_200_OK
        # Completing a conference releases the slot.
        from services.conferences.models import ConferenceSlot
