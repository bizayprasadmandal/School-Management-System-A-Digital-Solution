...
function CreateModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    if (!assignmentId) { Alert.alert("Validation", "Please select a class and subject."); return; }
    try {
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modal}>
          ...
          <ScrollView style={{ maxHeight: 450 }}>
            ...
            <TextInput value={title} onChangeText={setTitle} placeholder="e.g. Chapter 5 Homework" style={styles.input} placeholderTextColor="#94a3b8" />
            ...
            <View style={styles.row}>
              <View style={{ flex: 1, marginRight: 8 }}>
                ...
                <TextInput value={dueDate} onChangeText={setDueDate} style={styles.input} placeholderTextColor="#94a3b8" />
              </View>
              <View style={{ width: 80 }}>
                <Text style={styles.label}>Max Marks</Text>
                <TextInput value={maxMarks} onChangeText={setMaxMarks} keyboardType="numeric" style={styles.input} placeholderTextColor="#94a3b8" />
              </View>
            </View>
            ...
            <TouchableOpacity style={styles.filePicker} onPress={handlePickFile} activeOpacity={0.7}>
              {file ? (
                <View style={styles.fileSelected}>
                  ...
                  <TouchableOpacity onPress={() => setFile(null)}><Text style={styles.fileRemove}>Remove</Text></TouchableOpacity>
                </View>
              ) : (
            ...
            <TextInput value={description} onChangeText={setDescription} placeholder="Describe the assignment…" multiline style={[styles.input, { minHeight: 60 }]} placeholderTextColor="#94a3b8" />
          </ScrollView>

          <View style={styles.modalActions}>
            <TouchableOpacity style={styles.modalCancel} onPress={onClose}><Text style={styles.modalCancelTxt}>Cancel</Text></TouchableOpacity>
            <TouchableOpacity style={styles.modalConfirm} onPress={handleCreate}><Text style={styles.modalConfirmTxt}>Create</Text></TouchableOpacity>
          </View>
        </View>
      </View>
    </Modal>
...
function GradeModal({ assessment, onClose, onGraded }: { assessment: any; onClose: () => void; onGraded: () => void }) {
  ...
  const { data: subsData, isLoading } = useQuery({
    ...
    queryFn: () => mobileApi.get<{ results: any[] }>("/gradebook/submissions/", { assessment: assessment.id }),
  });
  const submissions = subsData?.results ?? [];

  const handleGrade = async (sub: any) => {
    const marks = parseFloat(grades[sub.id] as string);
    if (isNaN(marks) || marks < 0) { Alert.alert("Validation", "Enter valid marks"); return; }
    setSaving(sub.id);
    try {
    ...
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        <View style={[styles.modal, { maxHeight: 550 }]}>
          ...
          <Text style={styles.modalAssMeta}>{assessment.subject_name} · Max: {assessment.max_marks}</Text>

          {submissions.length === 0 ? (
            <EmptyState icon="📭" title="No submissions" sub="No student submissions for this assessment yet." />
          ) : (
                ...
                <View key={sub.id} style={styles.gradeRow}>
                  <View style={styles.gradeStudent}>
                    ...
                    {sub.marks_obtained != null ? (
                      <Text style={styles.gradedLabel}>✅ {sub.marks_obtained}/{assessment.max_marks}</Text>
                    ) : (
                  ...
                  {sub.marks_obtained == null ? (
                    <View style={styles.gradeInputs}>
                      ...
                      >
                        <Text style={styles.gradeBtnTxt}>{saving === sub.id ? "…" : "Grade"}</Text>
                      </TouchableOpacity>
                    </View>
                  ) : (
...
export default function TeacherAssignmentsScreen() {
  ...
  const { data: assData, isLoading, refetch } = useQuery({
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#f8fafc" }}>
      ...
      >
        ...
        {assessments.length > 0 && (
          <View style={styles.statsRow}>
            {[
              ...
              { label: "Homework", value: assessments.filter((a: any) => a.assessment_type === "homework").length, color: "#3b82f6", bg: "#eff6ff" },
              { label: "Quizzes", value: assessments.filter((a: any) => a.assessment_type === "quiz").length, color: "#8b5cf6", bg: "#f5f3ff" },
              { label: "Projects", value: assessments.filter((a: any) => a.assessment_type === "project").length, color: "#f59e0b", bg: "#fffbeb" },
        ...
        {/* List */}
        {assessments.length === 0 ? (
          <EmptyState icon="📚" title="No assignments yet" sub="Create your first assessment for your students." />
        ) : (
                ...
                <View key={ass.id} style={styles.card}>
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      <View style={styles.cardHeaderTop}>
                        <View style={[styles.typeBadge, { backgroundColor: TYPE_OPTIONS.find(o => o.value === ass.assessment_type)?.bg ?? "#f1f5f9" }]}>
                          <Text style={styles.typeIcon}>{TYPE_OPTIONS.find(o => o.value === ass.assessment_type)?.icon ?? "📋"}</Text>
                          <Text style={[styles.typeText, { color: TYPE_OPTIONS.find(o => o.value === ass.assessment_type)?.color ?? "#64748b" }]}>
                            {ass.assessment_type.charAt(0).toUpperCase() + ass.assessment_type.slice(1)}
                          </Text>
                        </View>
                      ...
                      <Text style={styles.cardMeta}>
                        {ass.subject_name} · {ass.classroom_name} · Due {dayjs(ass.due_date).format("MMM D")}
                      </Text>
                    </View>
                  ...
                  {expanded && (
                    <View style={styles.expanded}>
                      ...
                      {ass.attachment && (
                        <TouchableOpacity style={styles.attachBtn} onPress={() => Linking.openURL(ass.attachment)}>
      ...
      {gradeTarget && <GradeModal assessment={gradeTarget} onClose={() => setGradeTarget(null)} onGraded={onChanged} />}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  expanded: { padding: 14, paddingTop: 0, borderTopWidth: 1, borderTopColor: "#f1f5f9", paddingTop: 10 },
  desc: { fontSize: 13, color: "#475569", lineHeight: 18, marginBottom: 10 },
  attachBtn: { backgroundColor: "#eff6ff", borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8, marginBottom: 10, alignSelf: "flex-start" },
  attachBtnTxt: { fontSize: 12, fontWeight: "600", color: BRAND },
  gradeActionBtn: { backgroundColor: "#f59e0b", borderRadius: 8, paddingHorizontal: 14, paddingVertical: 10, alignItems: "center" },
  gradeActionTxt: { fontSize: 13, fontWeight: "700", color: "#fff" },
  // Modal styles
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "flex-end" },
  modal: { backgroundColor: "#fff", borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: Platform.OS === "ios" ? 40 : 20 },
  ...
  input: { borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 13, color: "#1e293b", marginBottom: 10, backgroundColor: "#fff" },
  chipRow: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: 10 },
  chip: { borderRadius: 16, borderWidth: 1, borderColor: "#e2e8f0", paddingHorizontal: 10, paddingVertical: 5, backgroundColor: "#fff" },
  chipActive: { backgroundColor: BRAND, borderColor: BRAND },
  chipTxt: { fontSize: 11, fontWeight: "600", color: "#64748b" },
  chipTxtActive: { color: "#fff" },
  row: { flexDirection: "row", alignItems: "flex-start" },
  filePicker: { borderWidth: 2, borderColor: "#e2e8f0", borderStyle: "dashed", borderRadius: 10, padding: 14, alignItems: "center", marginBottom: 10 },
  ...
  gradeRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
  gradeStudent: { flex: 1 },
  ...
  marksInput: { width: 50, borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 6, paddingHorizontal: 6, paddingVertical: 4, fontSize: 12, color: "#1e293b", textAlign: "center" },
  feedbackInput: { width: 80, borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 6, paddingHorizontal: 6, paddingVertical: 4, fontSize: 11, color: "#1e293b" },
