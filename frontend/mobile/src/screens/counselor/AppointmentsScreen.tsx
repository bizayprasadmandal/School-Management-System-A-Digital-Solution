...
const STATUS_COLORS: Record<string, string> = {
  scheduled: "#f59e0b", completed: "#059669", cancelled: "#ef4444", "in-progress": BRAND,
};

export default function AppointmentsScreen() {
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["mob-counselor-appointments"],
    queryFn: () => mobileApi.get<any>("/counseling/appointments/"),
  });
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}
      ...
      <Text style={styles.count}>{appointments.length} appointment{appointments.length !== 1 ? "s" : ""}</Text>

      {appointments.map((a: any) => (
        <Card key={a.id} style={styles.card}>
          <View style={styles.header}>
            <Text style={styles.student}>{a.student_name ?? a.student?.user?.full_name ?? "Student"}</Text>
            <Text style={[styles.status, { color: STATUS_COLORS[a.status] ?? "#64748b" }]}>{a.status}</Text>
          </View>
          <Text style={styles.date}>
            {a.scheduled_date ? new Date(a.scheduled_date).toLocaleDateString("en-US", {
