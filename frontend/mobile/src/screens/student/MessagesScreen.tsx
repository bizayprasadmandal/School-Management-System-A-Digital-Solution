...
export default function StudentMessagesScreen() {
  ...
  const { data: inbox, isLoading } = useQuery({
    queryKey: ["mob-student-inbox"],
    queryFn: () => mobileApi.get<any>("/communication/messages/inbox/"),
  });

  const { data: thread } = useQuery({
    queryKey: ["mob-student-thread", activeThread],
  ...
  const send = useMutation({
    ...
    onSuccess: () => { setMsg(""); qc.invalidateQueries({ queryKey: ["mob-student-thread", activeThread] }); },
  });
  ...
  if (!activeThread) return (
    <View style={{ flex: 1, backgroundColor: "#f8fafc" }}>
      ...
      {threads.length === 0
        ? <EmptyState icon="💬" title="No messages yet" sub="Messages from your teachers appear here." />
        : <FlatList data={threads} keyExtractor={i => i.partner.id}
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.thread} onPress={() => setActiveThread(item.partner.id)} activeOpacity={0.8}>
                <View style={styles.ava}><Text style={styles.avaTxt}>{item.partner.name.slice(0,2).toUpperCase()}</Text></View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.pName}>{item.partner.name}</Text>
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: "#f8fafc" }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <TouchableOpacity style={styles.back} onPress={() => setActiveThread(null)}>
      ...
      <View style={styles.bar}>
        ...
        <TouchableOpacity style={styles.sendBtn} onPress={() => msg.trim() && send.mutate(msg)}>
...
const styles = StyleSheet.create({
  ...
  ava: { width: 44, height: 44, borderRadius: 12, backgroundColor: "#e0e7ff", alignItems: "center", justifyContent: "center" },
  avaTxt: { fontSize: 13, fontWeight: "700", color: "#4338ca" },
  ...
  mine: { backgroundColor: "#6366f1", alignSelf: "flex-end" },
