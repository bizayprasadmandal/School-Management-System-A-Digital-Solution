...
async function setup(page: typeof test.prototype.page) {
  // ** globs tolerate the query strings the hooks append (?is_active=true&page=1, ?search=…&page=1, ?page=1)
  ...
  await page.route(`${API_BASE}/students/grades/`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify(MOCK_GRADES),
    });
  });
  await page.route(`${API_BASE}/academics/teacher-profiles/**`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
...
test.describe("Admin — Students Page", () => {
  ...
  test("renders student list with data", async ({ page }) => {
    ...
    // Two students are Active — pin the first badge
...
test.describe("Admin — Teachers Page", () => {
  ...
  test("shows add teacher button", async ({ page }) => {
    ...
    const addBtn = page
      .locator("a, button")
      .filter({ hasText: /Add Teacher|New Teacher/i })
      .first();
    await expect(addBtn).toBeVisible({ timeout: 5_000 });
  });
});
...
test.describe("Admin — Classrooms Page", () => {
  ...
  test("renders classroom list", async ({ page }) => {
    ...
    // Rows render "{grade_name} {name}"; the grade-filter option text is exactly "Grade 5",
    // so pin the row cell to avoid matching the <option> element
    await expect(page.getByText("Grade 5 5A")).toBeVisible({ timeout: 5_000 });
    await expect(page.getByText("Grade 6 6B")).toBeVisible();
    await expect(page.getByText("Grade 7 7A")).toBeVisible();
  });
