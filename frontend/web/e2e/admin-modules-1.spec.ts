...
// Dates must fall in the CURRENT month — the calendar filters events client-side
// by dayjs(currentMonth) so 2024 dates never render.
const NOW = new Date();
...
// Page renders emp.user_name / emp.employee_id / emp.status
...
test.describe("Admin — Library Page", () => {
  ...
  test("renders book catalog", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Library|Books/i })).toBeVisible({
      timeout: 5_000,
    });
...
test.describe("Admin — Behavior Page", () => {
  ...
  test("shows severity and status badges", async ({ page }) => {
    await gotoAdminPage(page, "behavior");
    await expect(page.getByText(/medium|low|resolved|pending/i).first()).toBeVisible({
      timeout: 5_000,
    });
  });
});
...
test.describe("Admin — Events Calendar Page", () => {
  ...
  test("shows create event button", async ({ page }) => {
    ...
    const createBtn = page
      .locator("button, a")
      .filter({ hasText: /Create|New|Add.*Event/i })
      .first();
...
test.describe("Admin — HR Page", () => {
  ...
  test("renders employees list", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /HR|Employee|Human Resource/i })).toBeVisible({
      timeout: 5_000,
    });
