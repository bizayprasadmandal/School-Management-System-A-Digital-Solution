...
const MOCK_ANNOUNCEMENTS = {
  count: 2,
  results: [
    { id: "1", title: "School Holiday", content: "School will be closed on Friday.", priority: "normal", audience: "all", is_draft: false, created_by_name: "Admin", created_at: "2024-06-01T10:00:00Z", published_at: "2024-06-01T10:00:00Z", view_count: 45 },
    { id: "2", title: "Exam Schedule", content: "Final exams start next week.", priority: "high", audience: "students", is_draft: false, created_by_name: "Admin", created_at: "2024-05-20T10:00:00Z", published_at: "2024-05-20T10:00:00Z", view_count: 120 },
  ],
};
...
const MOCK_CONFERENCE_SLOTS = {
  count: 2,
  results: [
    { id: "1", teacher_name: "Sarah Mitchell", start_time: "2024-06-15T14:00:00Z", end_time: "2024-06-15T14:30:00Z", is_booked: false, location: "Room 101" },
    { id: "2", teacher_name: "John Davis", start_time: "2024-06-15T15:00:00Z", end_time: "2024-06-15T15:30:00Z", is_booked: true, booked_by_name: "Alice Johnson", location: "Room 102" },
  ],
};
...
test.describe("Admin — Announcements Page", () => {
  ...
  test("renders announcement list with titles", async ({ page }) => {
    ...
    await expect(page.getByText(/Announcements/i)).toBeVisible({ timeout: 5_000 });
  ...
  test("shows new announcement button", async ({ page }) => {
    await gotoAdminPage(page, "announcements");
    const createBtn = page.locator("button, a").filter({ hasText: /New|Create|Add.*Announcement/i }).first();
...
test.describe("Admin — Conferences Page", () => {
  ...
  test.beforeEach(async ({ page }) => {
    await loginAsAdmin(page);
    await page.route(`${API_BASE}/conferences/slots/`, async (route) => {
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_CONFERENCE_SLOTS) });
    });
  ...
  test("renders conferences page with slot list", async ({ page }) => {
    ...
    await expect(page.getByText(/Conferences|Parent-Teach/i)).toBeVisible({ timeout: 5_000 });
    await expect(page.getByText("Room 101")).toBeVisible({ timeout: 5_000 });
  });
});
...
test.describe("Admin — Bulk Messages Page", () => {
  ...
  test("renders bulk messages page", async ({ page }) => {
    ...
    await expect(page.getByText(/Bulk|Messages|Communication/i)).toBeVisible({ timeout: 5_000 });
