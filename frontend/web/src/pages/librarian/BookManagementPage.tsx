...
function BookFormModal({
  ...
  const createMut = useMutation({
    mutationFn: (d: typeof form) =>
      api.post("/library/books/", { ...d, available_copies: d.total_copies }),
    onSuccess: () => {
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({
                  ...p,
                  total_copies: Math.max(1, parseInt(e.target.value) || 1),
                }))
              }
              className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm dark:text-slate-200"
            />
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-xl font-bold text-slate-900 dark:text-white">
            {totalAvailable + totalCheckedOut}
          </p>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                  ...
                  <button
                    onClick={() => {
                      if (confirm("Remove this book from the catalog?")) deleteMut.mutate(book.id);
                    }}
              ...
              {book.isbn && (
                <p className="text-xs text-slate-400 mt-1 font-mono">ISBN: {book.isbn}</p>
              )}
