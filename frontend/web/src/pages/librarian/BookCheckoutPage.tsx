...
function CheckoutFormModal({
  ...
  const { data: books = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get<{ results: any[] }>("/library/books/", {
        search: bookSearch,
        page_size: 10,
      });
      return res.results ?? [];
  ...
  const { data: students = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get<{ results: any[] }>("/students/", {
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span
                        className={`text-xs font-semibold ${
                          b.available_copies > 0 ? "text-green-600" : "text-red-500"
                        }`}
                      >
              ...
              {students.length === 0 ? (
                ...
                students.map((s: any) => (
                  <button
                    ...
                    onClick={() => {
                      setStudentId(s.id);
                      setStudentSearch(s.full_name ?? s.user?.full_name);
                    }}
                    className="w-full text-left px-3 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 dark:text-slate-200 border-b border-slate-100 dark:border-slate-700 last:border-0"
                  >
                    {s.full_name ?? s.user?.full_name}
                    <span className="text-xs text-slate-400 ml-2">
                      {s.admission_number || s.classroom?.name || ""}
                    </span>
                  </button>
                ))
              )}
...
export default function BookCheckoutPage() {
  ...
  const filtered =
    filter === "all"
      ? checkouts
      : filter === "active"
        ? checkouts.filter((c) => !c.returned_at && !c.is_overdue)
        : filter === "overdue"
          ? checkouts.filter((c) => !c.returned_at && c.is_overdue)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-xl font-bold text-green-600">
            {checkouts.filter((c) => c.returned_at).length}
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={
            filter === "active"
              ? "No active loans. Check out a book to get started."
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-slate-500 whitespace-nowrap">
                      {parseFloat(co.fine_amount) > 0 ? (
                        <span
                          className={
                            co.fine_paid ? "text-green-600 font-medium" : "text-red-500 font-medium"
                          }
                        >
