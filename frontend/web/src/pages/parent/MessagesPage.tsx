...
export default function ParentMessagesPage() {
  ...
  const { isTyping, sendTyping } = useChatSocket(
    ...
    () => {
      if (activeThread) {
        qc.invalidateQueries({ queryKey: ["parent-thread", activeThread] });
              ...
              {isLoading ? (
                ...
                <div className="p-6">
                  <EmptyState
                    ...
                    description="Start a conversation with a teacher from the school portal."
                  />
                </div>
              ) : (
            ...
            {!activeThread ? (
              ...
              <>
                    ...
                    >
                      <div
                        className={`max-w-xs rounded-2xl px-4 py-2.5 text-sm ${
                          m.is_mine ? "bg-violet-600 text-white" : "bg-slate-100 text-slate-800"
                        }`}
                      >
                        <p>{m.content}</p>
                        <p
                          className={`text-[10px] mt-1 ${
                            m.is_mine ? "text-violet-200" : "text-slate-400"
                          }`}
                        >
                ...
                <div className="border-t border-slate-100 p-3 flex gap-2">
                  {isTyping && <p className="w-full text-xs text-violet-500 px-1 pb-1">typing…</p>}
                  <textarea
                    rows={1}
                    value={msgText}
                    onChange={(e) => {
