...
export default function ParentFeesPage() {
  ...
  const { data: children } = useQuery({ queryKey: ["parent-children-fees"], queryFn: () => api.get<any>("/students/") });
  ...
  const { data: invData, isLoading } = useQuery({
    ...
    queryFn: () => api.get<any>(`/fees/invoices/?student=${child.id}`),
    enabled: !!child?.id,
  });
            ...
            <div className="card">
              ...
              {invoices.length === 0
                ...
                : <DataTable
                    ...
                    data={invoices as any[]} rowKey={r=>r.id}
                  />
              }
            </div>
          </>
