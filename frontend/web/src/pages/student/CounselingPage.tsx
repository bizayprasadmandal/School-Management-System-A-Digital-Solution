...
export default function CounselingPage() {
  ...
  const paginatedSessions = React.useMemo(() => {
    ...
    return sessions.slice(start, start + 12);
  }, [sessions, page]);

  const totalPages = Math.ceil(sessions.length / 12);

  const bulk = useBulkSelect(sessions);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        bulk.selectedArray.map((id) => api.delete("/counseling/sessions//" + id + "/")),
      );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <Pagination page={page} total={sessions.length} pageSize={12} onChange={setPage} />
      )}
      <Modal
        open={showForm}
        onClose={() => {
