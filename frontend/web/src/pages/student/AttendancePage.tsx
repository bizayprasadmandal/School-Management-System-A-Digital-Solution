...
export default function StudentAttendancePage() {
  ...
  const {
    ...
    queryFn: () =>
      ...
      }>(`/attendance/student-report/?student_id=${profile?.id}&month=${month}&year=${year}`),
    enabled: !!profile?.id,
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Track your daily attendance record</p>
      </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm dark:bg-slate-800 dark:border-slate-700 dark:shadow-none">
              <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between dark:border-slate-700">
                <h2 className="text-base font-semibold">
                  {dayjs(`${year}-${month}`).format("MMMM YYYY")} — Daily Record
                </h2>
              </div>
              <div className="p-5">
                ...
                <div className="grid grid-cols-7 gap-1">
                  ...
                  {Array.from(
                    {
                      length: (dayjs(`${year}-${String(month).padStart(2, "0")}-01`).day() + 6) % 7,
                    },
                    (_, i) => (
                      <div key={`pad-${i}`} />
                    ),
                  ...
                  {(report.records ?? []).map((rec: { date: string; status: string }) => {
                    ...
                    return (
                      <div
                        key={rec.date}
                        title={`${fmt.date(rec.date)}: ${ATTENDANCE_STATUS[s]?.label ?? s}`}
                        className={`rounded-lg p-2 text-center text-xs font-semibold cursor-default ${bg} ${
                          s === "P" || s === "E" ? "text-white" : "text-white"
                        }`}
                      >
                        {day}
                      </div>
