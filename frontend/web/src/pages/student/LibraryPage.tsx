...
export default function LibraryPage() {
  ...
  const PAGE_SIZE = 12;
  const infiniteItems = allBooks.slice(0, infinitePage * PAGE_SIZE);
  const infiniteHasMore = infiniteItems.length < allBooks.length;
  ...
  const paginatedBooks = React.useMemo(() => {
    ...
    return books.slice(start, start + 12);
  }, [books, page]);

  const totalPages = Math.ceil(books.length / 12);

  const bulk = useBulkSelect(books);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      bulk.clear();
      qc.invalidateQueries({ queryKey: ["student-library"] });
    } catch {
