...
function InfoCard({
  ...
  return (
    <div
      className={`rounded-xl p-4 ${accent} flex items-center gap-4 transition-transform hover:scale-[1.02] duration-200`}
    >
...
function EventItem({ event }: { event: SchoolEvent | Assessment }) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`text-sm font-medium truncate ${
            isPast ? "text-slate-500" : isToday ? "text-indigo-800" : "text-slate-800"
          }`}
        >
          {title}
        </p>
      ...
      {isToday && (
        <span className="h-2 w-2 rounded-full bg-indigo-500 animate-pulse flex-shrink-0 mt-1.5" />
      )}
    </div>
  );
}
...
export default function StudentDashboard() {
  ...
  const {
    ...
    isError: attError,
  } = useStudentAttendanceSummary(profile?.id ?? "", academicYear?.id);
  ...
  const pieData = useMemo(
    () =>
      [
        ...
        {
          name: "Outstanding",
          value: Math.max(feeSummary.totalOutstanding, 0),
        },
      ].filter((d) => d.value > 0),
    [feeSummary],
  );
  ...
  const upcomingEvents = useMemo(() => {
    ...
    if (events?.results) {
      schoolEvents.push(
        ...events.results.filter((e) => dayjs(e.start_date).isAfter(now.subtract(1, "day"))),
      );
    }
    if (assessments?.results) {
      schoolEvents.push(
        ...assessments.results.filter((a) => dayjs(a.due_date).isAfter(now.subtract(1, "day"))),
      );
    }
    return schoolEvents
      .sort((a, b) => {
        const dateA =
          "due_date" in a
            ? dayjs((a as Assessment).due_date)
            : dayjs((a as SchoolEvent).start_date);
        const dateB =
          "due_date" in b
            ? dayjs((b as Assessment).due_date)
            : dayjs((b as SchoolEvent).start_date);
      ...
      <div className="flex items-center justify-between">
        ...
        {attendanceSummary && (
          <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 rounded-lg px-3 py-1.5 border border-slate-200 dark:border-slate-700">
            ...
            Overall attendance:{" "}
            <span className="font-semibold" style={{ color: attendanceColor }}>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-xl bg-white dark:bg-slate-800 p-5 shadow-sm border border-slate-100 dark:border-slate-700">
                  ...
                  <div className="flex items-center gap-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 px-3 py-2 text-xs">
                    ...
                    <div>
                      <span className="text-amber-700 dark:text-amber-300">
                        Next due:{" "}
                        <strong>{dayjs(feeSummary.nextDue.due_date).format("MMM D, YYYY")}</strong>
                      </span>
            ...
            >
              ...
              {feeSummary.totalOutstanding > 0
                ? `Pay $${feeSummary.totalOutstanding.toLocaleString()}`
                : "View Fees"}
            </Link>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-800 dark:text-red-300">
                {feeSummary.overdue} Overdue Invoice
                {feeSummary.overdue > 1 ? "s" : ""}
              </p>
              <p className="text-xs text-red-600 dark:text-red-400">
                ${feeSummary.totalOutstanding.toLocaleString()} total outstanding. Late fees may
                apply.
              </p>
            </div>
          </div>
        ...
        {unread.length === 0 ? (
          ...
          <ul className="space-y-2">
            {unread.slice(0, 5).map((notif) => (
              <li
                key={notif.id}
                className="flex items-start gap-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 p-3 transition-colors hover:bg-slate-100 dark:hover:bg-slate-700"
              >
