...
export default function StudentTimetablePage() {
  ...
  const { data: profile } = useQuery({
    queryKey: ["student-me-tt"],
    queryFn: () => api.get<{ id: string; enrollments: { classroom: number }[] }>("/students/me/"),
  });
  ...
  const {
    ...
    queryFn: () =>
      api.get<
        ...
        >
      >(`/timetable/slots/weekly/?classroom_id=${classroomId}`),
    enabled: !!classroomId,
  });
  ...
  Object.values(weekly ?? {})
    ...
    .forEach(
      (s: {
        ...
        if (!subjectColorMap[s.subject_name])
          subjectColorMap[s.subject_name] = COLORS[colorIdx++ % COLORS.length];
      },
    );
            ...
            return (
              ...
              >
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between dark:border-slate-700">
                  ...
                  <Badge color="slate">{slots.length} periods</Badge>
                </div>
                      ...
                      (
                        ...
                        <div
                          ...
                          className={`rounded-xl border px-3 py-2.5 ${
                            subjectColorMap[s.subject_name] ?? COLORS[0]
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-semibold truncate">{s.subject_name}</p>
                            <p className="text-xs opacity-70 ml-2 flex-shrink-0">
                              {fmt.time(s.start_time)}
                            </p>
                          </div>
