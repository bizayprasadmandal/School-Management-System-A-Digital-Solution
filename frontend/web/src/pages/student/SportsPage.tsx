...
export default function SportsPage() {
  ...
  const createSport = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["student-sports"] });
      setShowForm(false);
    },
  });
  ...
  const handleExport = () => {
    ...
    const rows = filteredSports.map((row) => ({
      name: row.name ?? "",
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="mb-1 block text-sm font-medium">Min Players</label>
          <input
            type="number"
            min={1}
            value={f.min_players}
            onChange={(e) => setF((p) => ({ ...p, min_players: Number(e.target.value) }))}
            className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200"
          />
        </div>
        <div>
          ...
          <input
            ...
            min={1}
            value={f.max_players}
            onChange={(e) => setF((p) => ({ ...p, max_players: Number(e.target.value) }))}
            className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200"
          />
        </div>
      </div>
