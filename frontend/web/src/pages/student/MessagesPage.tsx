  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Direct messages with your teachers</p>
      </div>
      <div
              ...
              {isLoading ? (
                ...
                <div className="p-6">
                  <EmptyState
                    ...
                    description="Messages from your teachers appear here."
                  />
                </div>
              ) : (
            ...
            {!activeThread ? (
              ...
              <>
                    ...
                    >
                      <div
                        className={`max-w-xs rounded-2xl px-4 py-2.5 text-sm ${
                          m.is_mine ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-800"
                        }`}
                      >
                        <p>{m.content}</p>
                        <p
                          className={`text-[10px] mt-1 ${
                            m.is_mine ? "text-indigo-200" : "text-slate-400"
                          }`}
                        >
                ...
                <div className="border-t border-slate-100 p-3 flex gap-2">
                  {isTyping && <p className="w-full text-xs text-indigo-500 px-1 pb-1">typing…</p>}
                  <textarea
                    rows={1}
                    value={msgText}
                    onChange={(e) => {
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && !e.shiftKey) {
                        e.preventDefault();
                        msgText.trim() && sendMsg.mutate(msgText);
                      }
                    }}
