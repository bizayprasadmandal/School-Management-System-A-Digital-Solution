...
import { useStudentAssessments, useStudentSubmissions, useSubmitAssessment } from "../../api/hooks";
...
function typeBadge(type: AssessmentType) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold ${ASSESSMENT_COLORS[type]}`}
    >
...
function SubmitModal({ assessment, onClose }: { assessment: Assessment; onClose: () => void }) {
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {submission.submitted_at && (
            <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
              ...
              {submission.is_late && (
                <span className="rounded bg-rose-100 dark:bg-rose-900/30 px-2 py-0.5 text-xs font-medium text-rose-600 dark:text-rose-400">
                  Late
                </span>
              )}
            </div>
          ...
          {submission.file && (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 px-4 py-2 text-sm font-medium text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
            >
          ...
          {submission.remarks && (
            <div>
              ...
              <p className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
                {submission.remarks}
              </p>
            </div>
          )}
...
export default function StudentAssignmentsPage() {
  ...
  const filtered =
    filter === "all" ? categorized : categorized.filter((c) => c.category === filter);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Homework, quizzes, projects, and lab work
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <button
            ...
            className={`rounded-xl p-4 text-left ${color
              .split(" ")
              .slice(2)
              .join(" ")} transition-all hover:scale-[1.02] ${
              filter === label.toLowerCase() ? "ring-2 ring-indigo-500" : ""
            }`}
          >
              ...
              <div className="p-5">
                ...
                {assessment.attachment && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1.5 text-xs font-medium text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors mb-3"
                  >
      ...
      {/* Modals */}
      {submitTarget && (
        <SubmitModal assessment={submitTarget} onClose={() => setSubmitTarget(null)} />
      )}
      {detailTarget && (
