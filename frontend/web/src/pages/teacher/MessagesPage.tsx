...
export default function TeacherMessagesPage() {
  ...
  const sendMsg = useMutation({
    ...
    onSuccess: () => { setMsgText(""); qc.invalidateQueries({ queryKey: ["teacher-thread", activeThread] }); },
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Direct messages with students and parents</p>
      </div>
        ...
        <div className="flex h-full" style={{ minHeight: 480 }}>
          ...
          <div className="w-full sm:w-72 border-r border-slate-100 dark:border-slate-700 flex-shrink-0 flex flex-col">
            <div className="p-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Conversations</p>
              {threads.length > 0 && <span className="text-xs text-slate-400 dark:text-slate-500">{threads.length}</span>}
            </div>
            <div className="flex-1 overflow-y-auto">
              {isLoading ? (
                <div className="p-4"><SkeletonText lines={6} /></div>
              ) : inboxError ? (
                <div className="p-4"><ErrorState onRetry={() => refetchInbox()} /></div>
              ) : threads.length === 0 ? (
                <div className="p-6"><EmptyState icon={ChatBubbleLeftEllipsisIcon} title="No messages yet" description="Messages from students and parents appear here." /></div>
              ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-semibold text-slate-800 dark:text-white truncate">{t.partner.name}</p>
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 flex-shrink-0 ml-1">
                          {dayjs(t.last_message.sent_at).fromNow()}
                        </p>
                      </div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.last_message.content}</p>
                    </div>
                    {t.unread_count > 0 && (
          ...
          <div className="flex-1 flex flex-col">
            {!activeThread ? (
              ...
              <div className="flex-1 p-4"><ErrorState onRetry={() => refetchThread()} /></div>
            ) : (
              <>
                {/* Messages */}
                <div className="flex-1 overflow-y-auto p-4 flex flex-col-reverse gap-2">
                  {[...(thread ?? [])].reverse().map((m: ThreadMessage) => (
                    <div key={m.id} className={`flex ${m.is_mine ? "justify-end" : "justify-start"}`}>
                      ...
                      >
                        <p>{m.content}</p>
                        <p className={`text-[10px] mt-1 ${m.is_mine ? "text-indigo-200" : "text-slate-400 dark:text-slate-500"}`}>
                ...
                <div className="border-t border-slate-100 dark:border-slate-700 p-3 flex gap-2 bg-white dark:bg-slate-800">
                  ...
                  <Button
                    variant="primary"
                    size="sm"
                    onClick={() => { if (msgText.trim()) sendMsg.mutate(msgText); }}
