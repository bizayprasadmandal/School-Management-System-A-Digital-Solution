...
const STATUS_COLOR: Record<string, "slate" | "green" | "blue"> = {
  draft: "slate",
  approved: "green",
  completed: "blue",
};
...
export default function TeacherLessonPlansPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<any>("/academics/lesson-plans/"),
  });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage your teaching plans</p>
        </div>
        <Button
          variant="primary"
        ...
        <div className="space-y-3">
          {plans.map(
            (p: {
              ...
              <div
                key={p.id}
                className="bg-white rounded-2xl border border-slate-100 shadow-sm dark:bg-slate-800 dark:border-slate-700 dark:shadow-none p-4 hover:border-indigo-200 transition-colors"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500">
                      {p.subject_name} · {p.classroom_name} · {fmt.date(p.date)} ·{" "}
