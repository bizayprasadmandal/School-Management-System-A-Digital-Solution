...
export default function CounselingPage() {
  ...
  const paginatedReferrals = React.useMemo(() => {
    ...
    return referrals.slice(start, start + 12);
  }, [referrals, page]);

  const totalPages = Math.ceil(referrals.length / 12);

  const bulk = useBulkSelect(referrals);
  ...
  const handleExport = () => {
    ...
    downloadCsv(csv, "counseling-referrals-" + dayjs().format("YYYY-MM-DD") + ".csv");
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        bulk.selectedArray.map((id) => api.delete("/counseling/referrals//" + id + "/")),
      );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <Pagination page={page} total={referrals.length} pageSize={12} onChange={setPage} />
      )}
      <Modal
        open={showForm}
        onClose={() => {
