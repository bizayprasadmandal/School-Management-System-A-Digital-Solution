...
export default function SurveysPage() {
  ...
  const PAGE_SIZE = 12;
  const infiniteItems = allSurveys.slice(0, infinitePage * PAGE_SIZE);
  const infiniteHasMore = infiniteItems.length < allSurveys.length;
  ...
  const bulk = useBulkSelect(allSurveys);
  const [reorderMode, setReorderMode] = React.useState(false);
  const [orderedItems, setOrderedItems] = React.useState(allSurveys);

  React.useEffect(() => {
    setOrderedItems(allSurveys);
  }, [allSurveys]);
  ...
  const handleExport = () => {
    ...
    const rows = allSurveys.map((row) => ({
      title: row.title ?? "",
      description: row.description ?? "",
    }));
    const csv = toCsv(rows, cols);
    downloadCsv(csv, "surveys-" + dayjs().format("YYYY-MM-DD") + ".csv");
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        bulk.selectedArray.map((id) => api.delete("/counseling/surveys//" + id + "/")),
      );
