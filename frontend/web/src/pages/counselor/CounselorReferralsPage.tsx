...
const CATEGORIES = [
  ...
  {
    value: "behavior",
    label: "⚠️ Behavioral Concern",
    color: "bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300",
  },
  {
    value: "emotional",
    label: "❤️ Emotional / Mental Health",
    color: "bg-pink-100 text-pink-700 dark:bg-pink-900/40 dark:text-pink-300",
  },
  {
    value: "family",
    label: "🏠 Family / Home Issue",
    color: "bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300",
  },
  {
    value: "social",
    label: "👥 Social / Peer Issue",
...
function ReferralStatsBar({ referrals }: { referrals: StudentReferral[] }) {
  const pending = referrals.filter(
    (r) => r.status === "pending" || r.status === "under_review" || r.status === "contacted",
  ).length;
  const urgent = referrals.filter(
    (r) => r.priority === "urgent" && r.status !== "closed" && r.status !== "declined",
  ).length;
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <>
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm dark:text-slate-200 placeholder:text-slate-400"
                placeholder="Describe the outcome of the intervention..."
                autoFocus
              />
            </div>
            <div>
        ...
        {action === "close" && (
          <p className="text-sm text-slate-600 dark:text-slate-400 bg-amber-50 dark:bg-amber-900/30 p-3 rounded-lg">
            ...
            Closing this referral will mark it as resolved. Make sure all necessary actions have
            been documented.
          </p>
        )}
    ...
    <Modal open={open} onClose={onClose} title="Referral Details" size="md">
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span
            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${staConfig.color}`}
          >
            {staConfig.label}
          </span>
          <span
            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${priConfig.color}`}
          >
          ...
          <div>
            ...
            {(referral.student_grade || referral.student_class) && (
              <p className="text-xs text-slate-500">
                {referral.student_grade}
                {referral.student_grade && referral.student_class ? " — " : ""}
                {referral.student_class}
              </p>
            )}
          </div>
          <div>
        ...
        {referral.intervention_plan && (
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400 bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
              {referral.intervention_plan}
            </p>
          </div>
        )}
        ...
        {referral.outcome && (
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
              {referral.outcome}
            </p>
          </div>
        )}
        ...
        <p className="text-xs text-slate-400">
          Created {dayjs(referral.created_at).format("MMM D, YYYY [at] h:mm A")} ·{" "}
        ...
        <EmptyState
          ...
          description={
            search
              ...
              : "No referrals yet. Teachers can refer students here."
          }
          action={
            <Button onClick={() => setShowCreateForm(true)}>
              <PlusIcon className="h-4 w-4 mr-1" /> New Referral
            </Button>
          }
        />
      ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${staConfig.color}`}
                        >
                          {staConfig.label}
                        </span>
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${priConfig.color}`}
                        >
                          <priConfig.icon className="h-3 w-3" />
                          {priConfig.label}
                        </span>
                        {catConfig && (
                          <span
                            className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${catConfig.color}`}
                          >
                            {catConfig.label}
                          </span>
                        )}
