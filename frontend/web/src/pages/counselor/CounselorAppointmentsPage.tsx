...
import type {
  ...
  CounselingAppointmentType,
} from "../../types";
import {
  useCounselingAppointments,
  useCreateCounselingAppointment,
  useUpdateCounselingAppointment,
...
function TodaySchedule({
  ...
  const upcomingToday = todayApps.filter(
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <Badge color={upcomingToday.length > 0 ? "blue" : "slate"}>
          ...
          todayApps
            ...
            .map((app) => {
              ...
              return (
                <div
                  key={app.id}
                  className="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                      {APPOINTMENT_TYPES.find((t) => t.value === app.appointment_type)?.label ??
...
function StatsBar({
  ...
  stats: { today: number; upcoming: number; completed: number; total: number };
}) {
  return (
...
function AppointmentFormModal({
  ...
  open: boolean;
  onClose: () => void;
  appointment?: CounselingAppointment | null;
  onSaved: () => void;
}) {
  ...
  const [studentResults, setStudentResults] = useState<
    Array<{
      id: string;
      full_name: string;
      classroom?: { name: string; grade: { name: string } };
    }>
  >([]);
  ...
  return (
    <Modal
      ...
      description={
        isEdit
          ? "Update appointment details or add session notes"
          : "Book a counseling session with a student"
      }
      size="lg"
    >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((p) => ({
                  ...p,
                  appointment_type: e.target.value as CounselingAppointmentType,
                }))
              }
              className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm dark:text-slate-200"
            >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm dark:text-slate-200 placeholder:text-slate-400"
            placeholder="Document observations, progress, concerns, and action items..."
            autoFocus
          />
        </div>
        <div className="flex items-center gap-4">
    ...
    <Modal open={open} onClose={onClose} title="Appointment Details" size="md">
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${cfg.color}`}
          >
        ...
        {appointment.reason && (
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-700/30 p-3 rounded-lg">
              {appointment.reason}
            </p>
          </div>
        )}
          ...
          <div className="flex items-center gap-2 text-sm text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 p-3 rounded-lg">
            ...
            <span>
              ...
              {appointment.follow_up_date
                ? ` by ${dayjs(appointment.follow_up_date).format("MMM D, YYYY")}`
                : ""}
            </span>
          </div>
        )}
...
export default function CounselorAppointmentsPage() {
  ...
  const stats = useMemo(
    () => ({
      today: appointments.filter((a) => a.scheduled_date === today && a.status !== "cancelled")
        .length,
      upcoming: appointments.filter((a) => a.scheduled_date >= today && a.status === "scheduled")
        .length,
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter === "upcoming")
      items = items.filter(
        (a) => a.scheduled_date >= today && ["scheduled", "in_progress"].includes(a.status),
      );
  ...
  const handleCancel = useCallback(
    async (app: CounselingAppointment) => {
      if (
        !confirm(
          `Cancel appointment with ${app.student_name} on ${dayjs(app.scheduled_date).format(
            "MMM D",
          )}?`,
        )
      )
        ...
        <EmptyState
          ...
          description={
            search
              ...
              : "No appointments scheduled yet. Click &apos;New Appointment&apos; to book one."
          }
          action={
            <Button onClick={() => setShowForm(true)}>
              <PlusIcon className="h-4 w-4 mr-1" /> New Appointment
            </Button>
          }
        />
      ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                        {dayjs(app.scheduled_date).format("MMM D, YYYY")} ·{" "}
                        {dayjs(app.scheduled_time, "HH:mm").format("h:mm A")}
                        {app.duration_minutes ? ` · ${app.duration_minutes} min` : ""}
                        {app.location ? ` · 📍 ${app.location}` : ""}
                      </p>
                      {app.reason && (
                      ...
                      {app.follow_up_needed && (
                        <span className="inline-flex items-center gap-1 mt-1 text-xs text-amber-600 dark:text-amber-400">
                          ...
                          Follow-up{" "}
                          {app.follow_up_date
                            ? `by ${dayjs(app.follow_up_date).format("MMM D")}`
                            : "needed"}
                        </span>
                      )}
                    </div>
