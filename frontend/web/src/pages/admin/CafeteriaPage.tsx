...
export default function CafeteriaPage() {
  ...
  return (<div className="space-y-6">
    ...
    {tab==="menus"&&(mLoading?<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[1,2,3].map(i=><div key={i} className="h-24 animate-pulse bg-slate-100 dark:bg-slate-800 rounded-lg"/>)}</div>:
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{menus.map(m=><div key={m.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:shadow-md"><div className="flex items-start justify-between mb-1"><div><p className="font-semibold text-slate-900 dark:text-white">{m.name}</p><p className="text-xs text-slate-400">{dayjs(m.date).format("MMM D, YYYY")}</p></div><Badge color="indigo">{m.meal_type_display}</Badge></div>{m.items&&<p className="text-xs text-slate-500 mt-1">🍽️ {m.items}</p>}<p className="text-xs text-slate-400">${Number(m.price).toLocaleString()}</p>
        ...
        <div className="flex gap-2 mt-2 pt-2 border-t border-slate-100"><button onClick={()=>{setEditingMenu(m);setShowMenuForm(true)}} className="text-xs text-indigo-600 font-medium">Edit</button><button onClick={()=>{if(confirm("Delete?"))delMenu.mutate(m.id)}} className="text-xs text-red-500 font-medium">Delete</button></div></div>)}</div>)}
    {tab==="plans"&&(pLoading?<div className="grid gap-3 sm:grid-cols-2">{[1,2].map(i=><div key={i} className="h-24 animate-pulse bg-slate-100 dark:bg-slate-800 rounded-lg"/>)}</div>:
      ...
      <div className="grid gap-3 sm:grid-cols-2">{plans.map(p=><div key={p.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:shadow-md"><div className="flex items-start justify-between"><div><p className="font-semibold text-slate-900 dark:text-white">{p.name}</p><p className="text-lg font-bold text-indigo-600">${Number(p.price_per_period).toLocaleString()}<span className="text-sm font-normal text-slate-400">/{p.period_days} days</span></p></div><Badge color={p.is_active?"green":"slate"}>{p.is_active?"Active":"Inactive"}</Badge></div>{p.meals_included&&<p className="text-xs text-slate-400">Meals: {p.meals_included}</p>}
        <div className="flex gap-2 mt-2 pt-2 border-t border-slate-100"><button onClick={()=>{setEditingPlan(p);setShowPlanForm(true)}} className="text-xs text-indigo-600 font-medium">Edit</button><button onClick={()=>{if(confirm("Delete?"))delPlan.mutate(p.id)}} className="text-xs text-red-500 font-medium">Delete</button></div></div>)}</div>)}
