...
function AddAdminModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addAdmin.mutateAsync(form as Partial<SchoolAdminUser> & { password?: string });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl bg-white dark:bg-slate-800 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Leave blank for default (School@1234)"
              className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            />
          </div>
          <div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="rounded-lg border border-slate-300 dark:border-slate-600 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">Cancel</button>
            <button type="submit" disabled={addAdmin.isPending} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50 transition-colors">
  ...
  if (error || !school) {
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20 p-6 text-center">
        ...
        <button onClick={() => navigate("/admin/platform/schools")} className="mt-3 text-sm font-medium text-indigo-600 hover:text-indigo-700">
          Back to schools
        </button>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">{school.code} · {school.subdomain}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Students", value: school.student_count || 0, icon: AcademicCapIcon, color: "bg-emerald-500" },
          { label: "Teachers", value: school.teacher_count || 0, icon: UsersIcon, color: "bg-violet-500" },
          { label: "Revenue", value: formatCurrency(school.total_revenue || 0), icon: BanknotesIcon, color: "bg-amber-500" },
        ].map((stat) => (
          <div key={stat.label} className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5">
          <h2 className="text-base font-semibold text-slate-900 dark:text-white mb-4">School Details</h2>
          {isEditing ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditForm({ ...editForm, subscription_tier: e.target.value as School["subscription_tier"] })}
                  className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                >
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button onClick={() => setIsEditing(false)} className="rounded-lg border border-slate-300 dark:border-slate-600 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">Cancel</button>
                <button onClick={handleSave} disabled={updateSchool.isPending} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50 transition-colors">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: "Subscription", value: school.subscription_tier?.charAt(0).toUpperCase() + school.subscription_tier?.slice(1) },
              ].map((field) => (
                <div key={field.label} className="flex justify-between">
                  <span className="text-sm text-slate-500 dark:text-slate-400">{field.label}</span>
                  <span className="text-sm font-medium text-slate-900 dark:text-white text-right max-w-[60%] truncate">{field.value || "—"}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold text-slate-900 dark:text-white">School Admins</h2>
            <button
          ...
          {!adminsData?.results?.length ? (
            <p className="text-sm text-slate-400 dark:text-slate-500">No admins assigned to this school yet.</p>
          ) : (
              ...
              {adminsData.results.map((admin) => (
                <div key={admin.id} className="flex items-center justify-between py-3">
                  ...
                  <span className={`inline-flex items-center gap-1 text-xs font-medium ${
                    admin.is_active ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
                  }`}>
      ...
      <AddAdminModal schoolId={school.id} open={showAddAdmin} onClose={() => setShowAddAdmin(false)} />
    </div>
  );
}
