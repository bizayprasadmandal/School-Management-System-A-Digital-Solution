    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={() => mutate()} loading={isPending} disabled={!name || !gradeId}>
            Create Classroom
          </Button>
        </>
      }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{classrooms.length} sections configured</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm dark:bg-slate-800 dark:border-slate-700 dark:shadow-none p-4 flex gap-4">
        <Select
          ...
          options={grades.map((g) => ({ value: g.id, label: g.name }))}
          value={gradeFilter ?? ""}
          onChange={(e) => setGradeFilter(Number(e.target.value) || undefined)}
          className="w-48"
        />
      </div>
        ...
        {isLoading ? (
          ...
          <div className="p-8">
            <EmptyState
              ...
              title="No classrooms"
              description="Add classrooms to get started."
            />
          </div>
        ) : (
          <DataTable
            columns={[
              ...
              { key: "capacity", header: "Capacity" },
              {
                ...
                header: "Students",
                render: (r) => (
                  <Badge color={r.student_count >= r.capacity ? "red" : "green"}>
              ...
              },
              {
                ...
                render: (r) => r.teacher_name ?? <span className="text-slate-400">Unassigned</span>,
              },
              { key: "room_number", header: "Room", render: (r) => r.room_number || "—" },
              {
                ...
                render: (r) => (
                  <button
                    onClick={() => {
                      if (window.confirm("Delete this classroom?")) deleteMutation.mutate(r.id);
                    }}
      ...
      <ImportCsvModal
        ...
        endpoint="/students/classrooms/import-csv/"
        invalidateQueries={[["classrooms"]]}
        helpText={`grade_name,name,capacity,room_number,class_teacher_email,academic_year_name
Grade 5,A,40,201,jane.doe@school.edu
(grade_name and name are required; academic_year_name defaults to the current year; class_teacher_email must match an existing teacher)`}
      />
    </div>
  );
}
