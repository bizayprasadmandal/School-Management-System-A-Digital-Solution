...
export default function HealthPage() {
  ...
  return (<div className="space-y-6">
    ...
    {tab==="records"&&(rLoading?<div className="grid gap-3 sm:grid-cols-2">{[1,2].map(i=><div key={i} className="h-24 animate-pulse bg-slate-100 dark:bg-slate-800 rounded-lg"/>)}</div>:
      ...
      <div className="grid gap-3 sm:grid-cols-2">{records.map(r=><div key={r.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:shadow-md"><div className="flex items-start justify-between"><div><p className="font-semibold text-slate-900 dark:text-white">{r.student_name}</p><p className="text-xs text-slate-400">Blood: {r.blood_type}{r.emergency_contact_name?` · Contact: ${r.emergency_contact_name}`:""}</p></div><div className="flex gap-1 ml-4"><button onClick={()=>{setEditingRecord(r);setShowRecordForm(true)}} className="text-xs text-indigo-600 font-medium">Edit</button><button onClick={()=>{if(confirm("Delete?"))delRecord.mutate(r.id)}} className="text-xs text-red-500 font-medium ml-2">Delete</button></div></div>{r.allergies&&<p className="text-xs text-red-500 mt-1">⚠️ Allergies: {r.allergies}</p>}{r.chronic_conditions&&<p className="text-xs text-amber-600 mt-1">Conditions: {r.chronic_conditions}</p>}</div>)}</div>)}
    ...
    {tab==="immunizations"&&(iLoading?<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[1,2,3].map(i=><div key={i} className="h-20 animate-pulse bg-slate-100 dark:bg-slate-800 rounded-lg"/>)}</div>:
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{immunizations.map(i=><div key={i.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:shadow-md"><div className="flex items-start justify-between"><div><p className="font-semibold text-sm text-slate-900 dark:text-white">{i.vaccine_name}</p><p className="text-xs text-slate-400">{i.student_name} · Dose {i.dose_number}</p><p className="text-xs text-slate-400">{dayjs(i.date_administered).format("MMM D, YYYY")}{i.next_due_date?` · Next: ${dayjs(i.next_due_date).format("MMM D, YYYY")}`:""}</p></div><div className="flex gap-1 ml-4"><button onClick={()=>{if(confirm("Delete?"))delImm.mutate(i.id)}} className="text-xs text-red-500 font-medium">Delete</button></div></div></div>)}</div>)}
...
function RecordFormModal({open,onClose,record,students,onSaved}:{open:boolean;onClose:()=>void;record?:HealthRecord|null;students:Student[];onSaved:()=>void}){
  ...
  return(<Modal open={open} onClose={onClose} title={isEdit?"Edit Health Record":"Add Health Record"}><form onSubmit={submit} className="space-y-4">
    <div><label className="block text-sm font-medium mb-1">Student *</label><select value={f.student} onChange={e=>setF(p=>({...p,student:e.target.value}))} className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm"><option value="">Select...</option>{students.map(s=><option key={s.id} value={s.id}>{s.user_name}</option>)}</select></div>
...
function VisitFormModal({open,onClose,visit,students,onSaved}:{open:boolean;onClose:()=>void;visit?:NurseVisit|null;students:Student[];onSaved:()=>void}){
  ...
  return(<Modal open={open} onClose={onClose} title="Log Nurse Visit"><form onSubmit={submit} className="space-y-4">
    <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Student *</label><select value={f.student} onChange={e=>setF(p=>({...p,student:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm"><option value="">Select...</option>{students.map(s=><option key={s.id} value={s.id}>{s.user_name}</option>)}</select></div><div><label className="block text-sm font-medium mb-1">Visit Type</label><select value={f.visit_type} onChange={e=>setF(p=>({...p,visit_type:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm"><option value="sick">Sick Visit</option><option value="injury">Injury</option><option value="medication">Medication</option><option value="checkup">Routine Checkup</option><option value="followup">Follow-up</option><option value="other">Other</option></select></div></div>
...
function ImmFormModal({open,onClose,immunization,students,onSaved}:{open:boolean;onClose:()=>void;immunization?:Immunization|null;students:Student[];onSaved:()=>void}){
  ...
  return(<Modal open={open} onClose={onClose} title={isEdit?"Edit Immunization":"Add Immunization"}><form onSubmit={submit} className="space-y-4">
    <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Student *</label><select value={f.student} onChange={e=>setF(p=>({...p,student:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm"><option value="">Select...</option>{students.map(s=><option key={s.id} value={s.id}>{s.user_name}</option>)}</select></div><div><label className="block text-sm font-medium mb-1">Vaccine *</label><input value={f.vaccine_name} onChange={e=>setF(p=>({...p,vaccine_name:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm"/></div></div>
...
function MedFormModal({open,onClose,students,onSaved}:{open:boolean;onClose:()=>void;students:Student[];onSaved:()=>void}){
  ...
  return(<Modal open={open} onClose={onClose} title="Log Medication"><form onSubmit={submit} className="space-y-4">
    <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Student *</label><select value={f.student} onChange={e=>setF(p=>({...p,student:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm"><option value="">Select...</option>{students.map(s=><option key={s.id} value={s.id}>{s.user_name}</option>)}</select></div><div><label className="block text-sm font-medium mb-1">Medication *</label><input value={f.medication_name} onChange={e=>setF(p=>({...p,medication_name:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm"/></div></div>
