...
${slipsHtml}
  ...
  // ── Payroll vs budget (current academic year) ──
  ...
  useEffect(() => {
    let cancelled = false;
    api
      .get<BudgetData>("/hr/payslips/payroll-budget/")
      .then((res) => {
        ...
        >
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                FY {budget.academic_year} · salary &amp; payroll budget lines vs committed payslips
                (paid + approved).
              </p>
            </div>
            <div className="flex flex-wrap items-end gap-5">
              ...
              <div className="text-right">
                <p className="text-xs text-slate-500 dark:text-slate-400">Committed</p>
                <p className="text-lg font-bold tabular-nums text-slate-900 dark:text-white">
                  {fmtMoney(budget.committed)}
                </p>
              </div>
              <div className="text-right">
              ...
              <div className="text-right">
                ...
                <p className="text-lg font-bold tabular-nums text-slate-900 dark:text-white">
                  {budget.utilization === null ? "—" : `${budget.utilization}%`}
                </p>
              </div>
            </div>
          </div>
          {budget.utilization !== null && (
            <div
              ...
              aria-valuemin={0}
              aria-valuemax={100}
              aria-valuenow={Math.min(budget.utilization, 100)}
              aria-label="Budget utilization"
            >
              <div
                className={`h-full rounded-full ${
                  budget.utilization > 100 ? "bg-rose-500" : "bg-indigo-500"
                }`}
                style={{ width: `${Math.min(Math.max(budget.utilization, 0), 100)}%` }}
              />
            </div>
          )}
