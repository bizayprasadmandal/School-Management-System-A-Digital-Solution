...
import { Button, Avatar, Badge, DataTable, SkeletonTable, EmptyState, Modal, Input, Select } from "../../components/common";
...
function CreateTeacherModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    employee_id: "", date_of_birth: "", gender: "M",
    qualification: "bachelor", specialization: "", joining_date: "",
    experience_years: 0, department: "", address: "",
  });

  const { mutate, isPending } = useMutation({
    mutationFn: () => api.post("/academics/teacher-profiles/", {
      employee_id: form.employee_id, date_of_birth: form.date_of_birth || undefined,
      gender: form.gender, qualification: form.qualification,
      specialization: form.specialization, joining_date: form.joining_date || undefined,
      experience_years: form.experience_years, department: form.department,
      address: form.address,
    }),
    onSuccess: () => { toast.success("Teacher profile created!"); qc.invalidateQueries({ queryKey: ["teacher-profiles"] }); onClose(); },
    onError: (err: any) => toast.error(err?.message ?? "Failed to create teacher. Ensure the teacher user exists first (create via Django admin)."),
  });

  const set = <K extends keyof typeof form>(k: K, v: (typeof form)[K]) => setForm(f => ({ ...f, [k]: v }));

  return (
    <Modal open onClose={onClose} title="Add Teacher" size="lg"
      footer={<><Button variant="secondary" onClick={onClose}>Cancel</Button><Button onClick={() => mutate()} loading={isPending} disabled={!form.employee_id}>Create Teacher</Button></>}>
      <div className="space-y-4">
        <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-xs text-amber-800">
          Note: Teacher user account must exist first. Create the user with role=teacher via the Django admin panel before adding their profile here.
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Employee ID" value={form.employee_id} onChange={e => set("employee_id", e.target.value)} required />
          ...
          <Select label="Qualification" value={form.qualification} onChange={e => set("qualification", e.target.value)}
            options={[{value:"diploma",label:"Diploma"},{value:"bachelor",label:"Bachelor"},{value:"master",label:"Master"},{value:"phd",label:"PhD"}]} />
          ...
          <Input label="Experience (years)" type="number" value={form.experience_years} onChange={e => set("experience_years", Number(e.target.value))} />
...
export default function TeachersPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Teacher removed"); qc.invalidateQueries({ queryKey: ["teacher-profiles"] }); },
  });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-slate-900">Teachers</h1><p className="text-sm text-slate-500 mt-0.5">{data?.count ?? 0} staff members</p></div>
        <Button variant="primary" leftIcon={<PlusIcon className="h-4 w-4"/>} onClick={() => setShowCreate(true)}>Add Teacher</Button>
      </div>
          ...
          : <DataTable
              columns={[
                ...
                { key:"actions", header:"", render: r => (
                  <button onClick={() => { if(window.confirm("Remove this teacher?")) deleteMutation.mutate(r.id); }}
