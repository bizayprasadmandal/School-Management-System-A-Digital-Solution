...
export default function SportsPage() {
  ...
  return (<div className="space-y-6">
    ...
    {activeTab==="sports"&&(<>{sports.length>5&&<div className="relative max-w-sm"><MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400"/><input value={search} onChange={e=>setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm" placeholder="Search sports..."/></div>}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{filtered.map(s=><div key={s.id} onClick={()=>{setEditingSport(s);setShowSportForm(true)}} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-md transition-all cursor-pointer group"><div className="flex items-start justify-between mb-2"><div><p className="font-semibold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{s.name}</p><p className="text-xs text-slate-400">{s.category_display}{s.description?` · ${s.description}`:""}</p></div><Badge color={s.is_active?"green":"slate"}>{s.is_active?"Active":"Inactive"}</Badge></div><p className="text-xs text-slate-400 mb-2">{s.team_count} team{s.team_count!==1?"s":""}</p><div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-700" onClick={e=>e.stopPropagation()}><button onClick={()=>{setEditingSport(s);setShowSportForm(true)}} className="text-xs text-indigo-600 font-medium">Edit</button><button onClick={()=>{if(confirm("Delete?"))delSport.mutate(s.id)}} className="text-xs text-red-500 font-medium">Delete</button></div></div>)}</div>}</>)}
    {activeTab==="teams"&&(tLoading?<div className="grid gap-3 sm:grid-cols-2">{[1,2].map(i=><div key={i} className="h-24 animate-pulse bg-slate-100 dark:bg-slate-800 rounded-lg"/>)}</div>:
      ...
      <div className="grid gap-3 sm:grid-cols-2">{teams.map(t=><div key={t.id} onClick={()=>{setEditingTeam(t);setShowTeamForm(true)}} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-md transition-all cursor-pointer group"><div className="flex items-start justify-between mb-2"><div><p className="font-semibold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{t.name}</p><p className="text-xs text-slate-400">{t.sport_name} · {t.gender_display} · {t.member_count} members</p></div><Badge color={t.is_active?"green":"slate"}>{t.is_active?"Active":"Inactive"}</Badge></div>{t.coach_name&&<p className="text-xs text-slate-400">Coach: {t.coach_name}</p>}<div className="flex gap-2 mt-2 pt-2 border-t border-slate-100" onClick={e=>e.stopPropagation()}><button onClick={()=>{setEditingTeam(t);setShowTeamForm(true)}} className="text-xs text-indigo-600 font-medium">Edit</button><button onClick={()=>{if(confirm("Delete?"))delTeam.mutate(t.id)}} className="text-xs text-red-500 font-medium">Delete</button></div></div>)}</div>)}
...
function SportFormModal({open,onClose,sport,onSaved}:{open:boolean;onClose:()=>void;sport?:Sport|null;onSaved:()=>void}){
  ...
  return(<Modal open={open} onClose={onClose} title={isEdit?"Edit Sport":"Add Sport"}><form onSubmit={submit} className="space-y-4">
    <div><label className="block text-sm font-medium mb-1">Name *</label><input value={f.name} onChange={e=>setF(p=>({...p,name:e.target.value}))} className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm dark:text-slate-200" required/></div>
...
function TeamFormModal({open,onClose,team,sports,onSaved}:{open:boolean;onClose:()=>void;team?:Team|null;sports:Sport[];onSaved:()=>void}){
  ...
  return(<Modal open={open} onClose={onClose} title={isEdit?"Edit Team":"Add Team"}><form onSubmit={submit} className="space-y-4">
    <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Sport *</label><select value={f.sport} onChange={e=>setF(p=>({...p,sport:e.target.value}))} className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm" required><option value="">Select...</option>{sports.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></div><div><label className="block text-sm font-medium mb-1">Team Name *</label><input value={f.name} onChange={e=>setF(p=>({...p,name:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm" required/></div></div>
...
function EventFormModal({open,onClose,event,sports,teams,onSaved}:{open:boolean;onClose:()=>void;event?:SportEvent|null;sports:Sport[];teams:Team[];onSaved:()=>void}){
  ...
  return(<Modal open={open} onClose={onClose} title={isEdit?"Edit Event":"Add Event"}><form onSubmit={submit} className="space-y-4">
    <div><label className="block text-sm font-medium mb-1">Title *</label><input value={f.title} onChange={e=>setF(p=>({...p,title:e.target.value}))} className="w-full rounded-lg border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm" required/></div>
