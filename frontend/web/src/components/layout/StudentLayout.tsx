...
function SidebarShell({
  nav, accent, open, setOpen, user, logout, navigate, unread,
}: any) {
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden">
      ...
      <aside className={clsx(
        `fixed inset-y-0 left-0 z-30 flex w-64 flex-col ${accent} transition-transform duration-300 lg:static lg:translate-x-0`,
        ...
        <nav className="mt-4 flex-1 overflow-y-auto px-3 pb-4 space-y-0.5">
          {nav.map(({ label, to, icon: Icon }: any) => (
      ...
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        <header className="flex h-16 items-center justify-between bg-white px-4 shadow-sm border-b border-slate-200">
...
export function StudentLayout() {
  ...
  return <SidebarShell nav={STUDENT_NAV} accent="bg-blue-800" open={open}
    setOpen={setOpen} user={user} logout={logout} navigate={navigate} unread={unread} />;
}
...
export function ParentLayout() {
  ...
  const navigate = useNavigate();
  const { data: unread = 0 } = useUnreadNotificationCount();
  return <SidebarShell nav={PARENT_NAV} accent="bg-violet-800" open={open}
    setOpen={setOpen} user={user} logout={logout} navigate={navigate} unread={unread} />;
}
